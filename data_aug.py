...
def write_augmented_images_bboxes(i, old_image_name, image_ext, timage, bboxes, parent_folder, IMGS, LABELS, AUGMENTED_DATASET):
    ...
    label_basename = 'aug_' + old_image_name + '_' + str(idx) + '.txt'
...
aug_super_list = [    
    A.augmentations.transforms.CLAHE(clip_limit=6.0, tile_grid_size=(8, 8), always_apply=False, p=0.5), # yes
    A.augmentations.transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.Downscale (scale_min=0.30, scale_max=0.80, interpolation=None, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.Emboss (alpha=(0.2, 0.5), strength=(0.2, 0.7), always_apply=False, p=0.5), # yes
    A.augmentations.transforms.Equalize (mode='cv', by_channels=True, mask=None, mask_params=(), always_apply=False, p=0.5), # yes
    A.augmentations.transforms.FancyPCA (alpha=0.1, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.GaussianBlur (blur_limit=(3, 5), sigma_limit=0, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.GaussNoise (var_limit=(10.0, 50.0), mean=0, per_channel=True, always_apply=False, p=0.5), #yes
    # A.augmentations.transforms.GlassBlur (sigma=0.7, max_delta=4, iterations=2, always_apply=False, mode='fast', p=0.5), #no
    A.augmentations.transforms.HueSaturationValue (hue_shift_limit=20, sat_shift_limit=30, val_shift_limit=20, always_apply=False, p=0.5), #yes
    A.augmentations.transforms.MedianBlur (blur_limit=3, always_apply=False, p=0.5), #yes
    # A.augmentations.transforms.MotionBlur (blur_limit=7, always_apply=False, p=1), #no, yes-vehicle
    A.augmentations.transforms.MultiplicativeNoise (multiplier=(0.9, 1.1), per_channel=False, elementwise=False, always_apply=False, p=0.5), #yes
    ...
    A.augmentations.transforms.Posterize (num_bits=4, always_apply=False, p=0.5), #yes
    # A.augmentations.transforms.RandomBrightness (limit=0.2, always_apply=False, p=0.2), #yes
    A.augmentations.transforms.RandomBrightnessContrast (brightness_limit=0.2, contrast_limit=0.2, brightness_by_max=True, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.RandomContrast (limit=0.2, always_apply=False, p=0.5), # yes
    # A.augmentations.transforms.RandomFog (fog_coef_lower=0.3, fog_coef_upper=1, alpha_coef=0.08, always_apply=True, p=1), # no
    A.augmentations.transforms.RandomGamma (gamma_limit=(80, 120), eps=None, always_apply=False, p=0.5), # yes
    ...
    A.augmentations.transforms.RandomShadow (shadow_roi=(0, 0.5, 1, 1), num_shadows_lower=1, num_shadows_upper=2, shadow_dimension=5, always_apply=False, p=0.5), # yes
    ...
    A.augmentations.transforms.RandomToneCurve (scale=0.1, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.RGBShift (r_shift_limit=20, g_shift_limit=20, b_shift_limit=20, always_apply=False, p=0.5), #yes
    ...
    A.augmentations.transforms.UnsharpMask (blur_limit=(3, 7), sigma_limit=0.0, alpha=(0.2, 0.5), threshold=10, always_apply=False, p=0.5), # yes
    A.augmentations.transforms.HorizontalFlip(p=1), # yes
...
def data_augmentation(k, img_source, show_flag, seq_flag):
    ...
    for image_path in imagepaths:
        ...
        basename, image_ext = os.path.splitext(image_basename)
        ...
        if img_source.endswith('_part.txt'):
            startidx = 100
        else:
            startidx = 0
